...
def iDependency(colHeights):
    ...
    for i in range(2, 8):
        c, l, r = (
            ...
            colHeights[i - 1],
            colHeights[i + 1],
        )  # heights of current and adjacent columns
        if l - c >= 3 and r - c >= 3:
            iDep += 1
...
def analyze(board, cleared_lines, print_values_to_viewer=False):
    ...
    return (
        -(varA * weights_editable[0])
        + varB * weights_editable[1]
        - varC * weights_editable[2]
        - varD * weights_editable[3]
        + varE * weights_editable[4]
        - varF * weights_editable[5]
        - varG * weights_editable[6]
        + PCbonus * weights_editable[7]
        + cleared_lines * weights_editable[8]
    )
